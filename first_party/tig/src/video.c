    ...
    if ((blit_info->flags & TIG_VIDEO_BUFFER_BLIT_BLEND_COLOR_LERP) != 0) {
        ...
        if (video_simd_blit_enabled()) {
            ...
            for (sy = 0; sy < blit_dst_rect.height; ++sy) {
                ...
                // This path indexes src[sx]/dst[sx] (pointers NOT advanced inside the
                // row), so the next row is simply +pitch. Do NOT use the scalar loop's
                // pitch - 4*width (correct only when the pointers ARE incremented) or
                // rows after the first read the wrong location -> black/garbage.
                src = (uint32_t*)((uint8_t*)src + blit_info->src_video_buffer->surface->pitch);
                dst = (uint32_t*)((uint8_t*)dst + blit_info->dst_video_buffer->surface->pitch);
            }
            return TIG_OK;
        }
#endif
