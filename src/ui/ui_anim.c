...
static void ui_anim_finalize(ui_anim_slot_t* s)
{
    ...
    if (s->kind == UI_ANIM_KIND_WINDOW_TRANSFORM) {
        ...
        // Release the pre-baked tint snapshot. The natural tint path
        // takes over after transform_clear (or stays in transform
        // territory for fully-hidden states — either way the
        // snapshot's role is done).
