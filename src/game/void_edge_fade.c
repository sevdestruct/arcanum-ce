...
// A gap is anything that would render black: the terrain_fill placeholder, an
// authored void tile, or (DEFAULT-OFF now) genuinely missing art.
//
// By default we SKIP the !gap_renderable() check, whose gap_art_exists_cached()->
// tig_art_exists() does a ~46us file stat across the asset override tiers -- the
// dominant cost of a cold sector load (gap_vmask_store scans all 4096 tiles). For a
// normal install ALL tile art exists, so that check never actually finds a gap; the
// real void edges are the bit-checkable placeholder/void-black tiles. Skipping it
// makes the per-sector void scan ~free. Tradeoff: a genuinely missing-art tile
// (corrupt install / a mod that omits art) renders black but no longer feathers --
// set ARCANUM_OPT_VOIDFADE=0 to restore the file-check (safe) path if that matters.
static bool gap_is_gap(tig_art_id_t aid)
{
    static int skip_exist = -1;
    if (skip_exist < 0) {
        const char* e = getenv("ARCANUM_OPT_VOIDFADE");
        skip_exist = (e != NULL && e[0] == '0') ? 0 : 1; // default ON; =0 restores the stat
    }
    if (skip_exist) {
...
static void gap_vmask_store(int64_t id, Sector* sector)
{
    ...
    // Per-sector dedup of gap_is_gap(): a sector's 4096 tiles repeat only a few hundred
    // unique arts. Always on (correctness-neutral; gap_is_gap is per-art-id deterministic).
    // It matters most for the ARCANUM_OPT_VOIDFADE=0 safe path, where gap_is_gap() still
    // does the ~46us tig_art_exists() file stat -- deduping caps that at the unique-art
    // count instead of all 4096 tiles. (The default path skips the stat, so it's ~free.)
