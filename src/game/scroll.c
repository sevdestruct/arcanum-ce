...
void scroll_by(int64_t dx, int64_t dy)
{
    ...
    /* When fog is active at 1:1 zoom, skip the hardware pixel-copy scroll.
     * tig_window_scroll would physically shift the already-fogged pixels and
     * then fog would be re-applied on top of the shifted fogged content,
     * producing hard-edged artifacts and stale NPC rectangles. */
    if (z != 1.0f || tb_any_active() || perception_fog_is_enabled()) {
