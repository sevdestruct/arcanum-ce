...
// CE (feature/perf-gpu-accel): the autonomous arbiter test command channel now
// lives entirely in harness.c (harness_channel_tick, pumped from gamelib_ping
// below). These two small accessors expose the file-private statics it needs so
// all harness logic can live in one file (see docs/arbiter-harness.md phase 2).
#if defined(ARCANUM_HARNESS)
void gamelib_zoom_perf_set_warmed_up(bool warmed_up)
{
    gamelib_zoom_perf_warmed_up = warmed_up;
}

void gamelib_render_path_set(const char* mode)
{
    settings_set_str_value(&settings, RENDER_PATH_KEY, mode);
}
...
void gamelib_ping(void)
{
    ...
    // CE: pump the autonomous arbiter test channel (harness.c) before module
    // pings so a loadsave command takes effect this same frame.
#if defined(ARCANUM_HARNESS)
    harness_channel_tick();
#endif
